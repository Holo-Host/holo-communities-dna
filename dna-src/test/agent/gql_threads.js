...
scenario('Check for a non existent thread and then create it', async (s, t) => {
  const { alice } = await s.players({alice: one('alice')}, true)
    ...
    const result = await alice.callSync("app", "graphql", "graphql", {
    ...
    // add a thread
    const add_result_str = await alice.callSync("app", "graphql", "graphql", {
    ...
    t.assert(add_result.findOrCreateThread.participants
      ...
      .includes(alice.info('app').agentAddress)
    )

    const get_result_post = await alice.callSync("app", "graphql", "graphql", {
      query: queries.getMessageThreadsQuery,
      variables: {first: 10, offset: 0}
    })
